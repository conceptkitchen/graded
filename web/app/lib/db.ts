...
function getSQL() {
  if (!DATABASE_URL) {
    console.warn("DATABASE_URL not set — learned patterns will not persist");
    return null;
  }
...
/**
 * Get all learned patterns from the database.
 */
 ...
 * Log a sync event.
 */
export async function logSync(upstreamCount: number, localCount: number, newPatterns: number, source = "augustus"): Promise<void> {
  const sql = getSQL();
  if (!sql) return;

  try {
    await sql`
      INSERT INTO sync_log (upstream_count, local_count, new_patterns, source)
      VALUES (${upstreamCount}, ${localCount}, ${newPatterns}, ${source})
    `;
  } catch (e) {
